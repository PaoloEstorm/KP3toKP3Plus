...
The patched firmware brings all the features of the **Kaoss Pad 3+** to the Kaoss Pad 3, with no compromises.
...
**Note for Mac users:** Unfortunately, the update procedure cannot be performed on macOS, since the mac version of the KP3 Updater does not offer an option to select a custom `.VSB` file. (A possible workaround would be to replace the `KP3Plus.VSB` file inside the app — `KP3+ Updater.app/Contents/Resources` — with the file generated by the tool, but I haven't tested it)
Also, the KP3+ Editor is no longer runnable, since 32-bit applications are no longer supported on macOS as of macOS Catalina (10.15).
