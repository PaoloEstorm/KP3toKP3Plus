...
**Note for Mac users:** Unfortunately, the update procedure cannot be performed on macOS, since the KP3+ Updater does not offer an option to select a custom `.VSB` file. (A possible workaround would be to replace the `KP3Plus.VSB` file inside the app — `KP3+ Updater.app/Contents/Resources` — with the file generated by the tool, but I haven't tested it)
