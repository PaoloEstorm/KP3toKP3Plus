...
This is a guide to make a patched firmware for the **Korg Kaoss Pad 3**, to be installed via Korg's official updater.
The patched firmware brings all the features of the **Kaoss Pad 3+** to the Kaoss Pad 3.
...
***Note:** If the procedure completed successfully, your device has effectively become a KP3+. So, if you plan to use the Editor application or to use it as a MIDI-USB controller, you'll need the drivers and the KP3+ Editor, both downloadable form the [official support page](https://www.korg.com/us/support/download/product/0/124/).
 
**Note for Mac users:** The `KP3 Updater` & `KP3/KP3+ Editor` are no longer runnable, since 32-bit applications are no longer supported on macOS as of macOS Catalina (10.15).
